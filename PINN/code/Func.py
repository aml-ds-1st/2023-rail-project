...
def Train(model, model_type, train_DL, val_DL, criterion, optimizer, 
    ...
    for ep in range(EPOCH): 
        ...
        print(f"Epoch: {ep+1}, current_LR = {current_lr:.6f}")
    ...
    torch.save({"loss_history": loss_history,
                ...
                "memo":memo}, save_history_path)
    
    torch.save({"model":model,
                "ep":ep,
                "optimizer":optimizer,
                "scheduler":scheduler,
                "memo":memo}, save_model_path[:-3]+"_FIN"+save_model_path[-3:])
...
def loss_epoch(model, model_type, DL, criterion, optimizer = None, d_max=None , d_min=None):
    ...
    for x_batch, y_batch in tqdm(DL, leave=False):
        ...
        # loss
        if model_type=="PINN":
            Gloss= gov_loss3(x_batch ,y_batch, y_hat, d_max, d_min)
            loss = criterion(y_hat, y_batch[:,0])+ Gloss
        elif model_type=="OnlyPINN":
            Gloss= gov_loss3(x_batch ,y_batch, y_hat, d_max, d_min)
            loss = Gloss
        else:
            loss = criterion(y_hat, y_batch[:,0])
        # update
...
def gov_loss2(x_batch , y_batch, y_hat, y_init, d_max, d_min):
    ...
    y_real=(d_min[-1]+(d_max[-1]-d_min[-1])*y_batch[:,0])
    y_hat=(d_min[-1]+(d_max[-1]-d_min[-1])*y_hat[:,0])
...
def gov_loss3(x_batch , y_batch, y_hat, d_max, d_min):
    h_conv=x_batch[:,8]*0.02;   c=0.444;      m=2;        epsilon=0.8
    A_s=0.011;  sigma= 5.67*10**-8;     del_t=1/6
     
    x_real_air=(d_min[0]+(d_max[0]-d_min[0])*x_batch[:,0]) 
    x_real_solar=(d_min[5]+(d_max[5]-d_min[5])*x_batch[:,5]) 
    
    y_real_past=(d_min[-2]+(d_max[-2]-d_min[-2])*y_batch[:,-1])
    y_hat=(d_min[-2]+(d_max[-2]-d_min[-2])*y_hat[:,0])

    Q1=(x_real_solar+ h_conv  * (y_hat- x_real_air)) * del_t * A_s
    Q2=c * m * (y_hat - y_real_past)
    Gloss=((Q1-Q2)**2).mean()
    
    return Gloss

def gov_loss4(x_batch , y_batch, y_hat, d_max, d_min):
    h_conv=x_batch[:,8]*0.02;   c=0.490;      m=7850;        epsilon=0.8
    sigma= 5.67*10**-8;     del_t=1/6
    ...
    Q1=(x_real_solar+ h_conv  * (y_hat- x_real_air)) * del_t 
    Q2=c * m * (y_hat - y_real_past)
    Gloss=((Q1-Q2)**2).mean()
